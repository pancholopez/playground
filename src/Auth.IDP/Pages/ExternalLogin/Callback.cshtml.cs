...
namespace Auth.IDP.Pages.ExternalLogin;
...
public class Callback : PageModel
{
    ...
    // TODO: figure out how to do un-linking
    public async Task<IActionResult> OnGet()
    {
        ...
        // find external user
        var user = await _localUserService.FindByExternalProvider(provider, providerUserId);
        if (user == null)
        {
            ...
            if (provider == "AAD")
            {
                ...
                var emailFromAzureAD = claims.FirstOrDefault(c =>
            ...
            else if (provider == "Facebook")
            {
                ...
                if (!string.IsNullOrEmpty(email))
                {
                    ...
                    // if user does not exist, provision new user
                    if (user == null)
                    {
                    ...
                    else // if it exists, add AAD as a provider
                    {
                        await _localUserService.AddExternalProviderToUser(
                            user.Subject, provider, providerUserId);
                        await _localUserService.SaveChangesAsync();
